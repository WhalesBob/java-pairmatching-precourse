...
1. 기능을 선택하는 부분의 입출력
   - 출력 : 
     - 기능을 입력받고, 페어 매칭, 페어 조회, 페어 초기화, 종료 순으로 받는다
   - 입력
     - 1,2,3,Q 만 받을 수 있으며, 다른 부분이 나올 시 에러 처리하고 다시 입력받는다.
     - 이때 반드시, "[ERROR]" 를 포함한 에러 메세지를 출력받는다. 

2. 페어 매칭 시 입출력
   - 출력 : 
     - 위아래로 "#############################################" 를 붙인다
     - 과정은 백앤드 | 프론트앤드 로 보여주고
     - 미션은, 레벨 1부터 5까지 제공한다
     - 이후 과정, 레벨, 미션을 선택하라는 문구를 보여 주고, ex) 부분을 보여 준다.
     - 이후, 페어 매칭 결과를 출력하는 식으로 이어간다.
   - 입력
     - 순서대로 백앤드, 레벨1, 자동차경주 순으로 입력받는다
     - 만약에, 없는 것을 입력했을 시, 에러 메세지를 출력하고, 새로 입력을 받는다. 

3. 자바 파일 입출력
   - 주어진 md 파일을 읽어서 저장해야 한다.
...
- 처음에 코스(프론트앤드, 백앤드), 레벨(1,2,3,4,5레벨), 코스 내용은 enum 으로 구성한다
    - 각 코스는 enum 으로 구성하고, 각 enum 내에 학생 List 를 두어 관리한다
    - 각 레벨도 enum 으로 구성하고, 각 enum 내에 레벨 별 과정을 관리한다.
